        ...
        case GGML_OP_DSV4_HC_COMB:
            {
                ...
                ggml_metal_encoder_dispatch_threadgroups(
                        enc, (args.n_tokens + nsg - 1)/nsg, 1, 1, GGML_METAL_NW, nsg, 1);
            } break;
        case GGML_OP_DSV4_HC_PRE:
            {
                ...
                ggml_metal_encoder_dispatch_threadgroups(
                        enc, (n_tiles + nsg - 1)/nsg, args.n_tokens, 1, GGML_METAL_NW, nsg, 1);
            } break;
        case GGML_OP_DSV4_HC_POST:
            {
                const ggml_tensor * x        = op->src[0];
...
int ggml_metal_op_fwht(ggml_metal_op_t ctx, int idx) {
    ...
    const int simd_size = GGML_METAL_NW;
...
int ggml_metal_op_mul_mat(ggml_metal_op_t ctx, int idx) {
    ...
    // batch size above which the mat-mul kernel beats the small-batch mat-vec ones.
    // tunable for experiments (e.g. speculative decoding verifies only a few tokens at a time)
    static const int ne11_mm_min = getenv("GGML_METAL_MM_MIN") ? atoi(getenv("GGML_METAL_MM_MIN")) : 8;
    ...
    if (op->src[1]->type == GGML_TYPE_F32 && (ne00%128 == 0) &&
        ...
        (props_dev->has_simdgroup_mm || props_dev->has_mm_w64) && ne00 >= 64 && ne11 > ne11_mm_min) {
...
#define FATTN_SMEM(nsg) (GGML_PAD((nqptg*(ne00 + 2*GGML_PAD(ne20, 64) + 2*(2*ncpsg)) + is_q*(16*32*(nsg)))*(sizeof(float)/2), 16))
        ...
        ggml_metal_encoder_dispatch_threadgroups(enc, (ne01 + nqptg - 1)/nqptg, ne02, ne03, GGML_METAL_NW, nsg, 1);
#undef FATTN_SMEM
    } else {
        // half4x4 kernel
...
#define FATTN_SMEM(nsg) (GGML_PAD(((GGML_PAD(ne00, 128) + 4*ncpsg + 2*GGML_PAD(ne20, 128))*(nsg))*(sizeof(float)/2), 16))
        ...
        if (nwg == 1) {
            ...
            ggml_metal_encoder_dispatch_threadgroups(enc, (ne01 + nqptg - 1)/nqptg, (ne02 + nhptg - 1)/nhptg, ne03*nwg, GGML_METAL_NW, nsg, 1);
        } else {
            // sanity checks
            assert(ggml_metal_op_flash_attn_ext_extra_tmp(op) != 0);
